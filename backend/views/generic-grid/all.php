...
?>
    ...
    margin-top: 1px;margin-right: 5px;float: left;"  data-toggle="modal" data-target="#exampleModal" data-whatever="@mdo">Advanced Filters</button>-->
                                ...
                                <input type="hidden" value="10" id="hidden_page_per_record">
                            </div>
                                    ...
                                    foreach ( $config['thead'] as $key=>$value ){
                                        ...
                                        <th <?=$persist?> scope="col" data-tablesaw-priority="<?=$counter?>" class="min-th-width footable-sortable tablesaw-sortable-ascending" ><?=$key?>
                                            ...
                                            <input type="<?=$inputtype?>" data-filter-field="<?=$value['data-filter-field']?>" data-filter-type="<?=$value['data-filter-type']?>"
                                                    class=" inputs-margin filters-visible filter form-control <?=$value['input-type-class']?>" value="">
                                                <?php
                                            ...
                                            <select data-filter-field="<?=$value['data-filter-field']?>" data-filter-type="<?=$value['data-filter-type']?>"
                                                    class="<?=$value['input-type-class']?> inputs-margin filters-visible filter form-control select-filter">
                                                <option></option>
                                                <?php
                                                foreach ( $value['options'] as $valuees ){
                                                    ?>
                                                    <option <?=(isset($_GET['performance']) && $_GET['performance']!='' &&
                                                        (ucfirst($_GET['performance'])==$valuees['value']) )
                                                    ? 'selected' : ''?>
                                                            value="<?=$sign.$valuees['key']?>"><?=$valuees['value']?></option>
                                                    <?php
                                                }
                                                ?>
                                            </select>
        ...
        <script type="text/javascript">
            ...
            var entitytype = '<?= isset($config['UrlSetting']['entitytype']) ? $_GET['entitytype'] : '' ?>';
            var performance = '<?= isset($config['UrlSetting']['performance']) ? $_GET['performance'] : '' ?>';
        </script>
    </div>
