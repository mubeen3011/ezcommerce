        ...
        */
        ...
        <tr <?=$style?>  <?=($pdsKey==0) ? 'class="Bundle-Div"' : '' ?> <?=($pdsKey==0) ? 'id="Bundle-'.$bundle_info['id'].'"' : ''?>>
            ...
            <td class="tg-kr94 sku_td">
                ...
                <i class="fa fa-info-circle"  data-toggle="modal" data-target="#extra-information-popup" onclick="ShowExtraInformation('<?=$pd['sku_id']?>','fbl-909-avent')" style="font-size: 15px;cursor: pointer;color: orange;">
            ...
            <td class="tg-kr94 current_stock_td"><?= ($pd['avent_stks'] + $pd['avent_stocks_intransit']) - $pd['avent_stocks_intransit']; ?>
                <input type="hidden" value="<?= $pd['avent_stks']; ?>" name="bundle[cs_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
            </td>
            <td class="tg-kr94 stock_in_transit_td">
                <?= ($pd['avent_stocks_intransit']=='') ? 0 : $pd['avent_stocks_intransit']; ?>
                <input type="hidden" value="<?= ($pd['avent_stocks_intransit']=='') ? 0 : $pd['avent_stocks_intransit']; ?>" name="bundle[si_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
            </td>
            ...
            <td class="tg-kr94 suggested_order_qty_td"><?= $pd['avent_order_stocks']; ?>
                <input type="hidden" value="<?= $pd['avent_order_stocks']; ?>" name="bundle[soq_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
            </td>
