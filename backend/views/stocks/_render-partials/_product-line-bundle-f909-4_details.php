    ...
    foreach ($refine['c'] as $pdsKey=>$pd) :
        ...
        */
        ...
        if (isset($pod['name']) && !in_array($pd['sku_id'], $pod['name']) && !empty($pod))
            continue;
        $orderQty = $pd['fbl909_order_stocks'];
        $pd['909_stks'] = $pd['909_stks'];
        ...
        <tr <?=$style?>  <?=($pdsKey==0) ? 'class="Bundle-Div"' : '' ?> <?=($pdsKey==0) ? 'id="Bundle-'.$bundle_info['id'].'"' : ''?>>
            ...
            <td class="tg-kr94 sku_td">
                ...
                <i class="fa fa-info-circle"  data-toggle="modal" data-target="#extra-information-popup" onclick="ShowExtraInformation('<?=$pd['sku_id']?>','fbl-909-4')" style="font-size: 15px;cursor: pointer;color: orange;">
            ...
            <td class="tg-kr94 threshold_td"><?= ($pd['threshold_org'] != "") ? $pd['threshold_org'] : 0; ?>
                <input type="hidden" value="<?= ($pd['threshold_org'] != "") ? $pd['threshold_org'] : 0; ?>" name="bundle[th_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]"></td>
            <td>
            ...
            <td class="tg-kr94 current_stock_td"><?= ($pd['909_stks'] - $pd['fbl909_stocks_intransit']) - $pd['fbl909_stocks_intransit']; ?>
                <input type="hidden" value="<?= $pd['909_stks']; ?>" name="bundle[cs_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
            </td>
            <td class="tg-kr94 stock_in_transit_td">
                <?= ($pd['fbl909_stocks_intransit']=='') ? 0 : $pd['fbl909_stocks_intransit']; ?>
                <input type="hidden" value="<?= ($pd['fbl909_stocks_intransit']=='') ? 0 : $pd['fbl909_stocks_intransit']; ?>" name="bundle[si_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
            </td>
            ...
            <td class="tg-kr94 suggested_order_qty_td"><?= $pd['fbl909_order_stocks']; ?>
                <input type="hidden" value="<?= $pd['fbl909_order_stocks']; ?>" name="bundle[soq_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
            </td>
