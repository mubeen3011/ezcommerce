    ...
    foreach ($refine['a'] as $pdsKey=>$pd) :
        ...
        */
                ...
                if (isset($pod['name']) && !in_array($pd['sku_id'], $pod['name']) && !empty($pod))
                    continue;
                $orderQty = $pd['isis_order_stocks'];
                $pd['isis_stks'] = $pd['isis_stks'];
                ...
                <tr <?=$style?>  <?=($pdsKey==0) ? 'class="Bundle-Div"' : '' ?> <?=($pdsKey==0) ? 'id="Bundle-'.$bundle_info['id'].'"' : ''?>>
                    ...
                    <td class="status_td"><?=$pd['stock_status']?></td>
                    ...
                    <td class="tg-kr94 threshold_td"><?= $pd['threshold'] ?>
                        <input type="hidden" value="<?= $pd['threshold']; ?>" name="bundle[th_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]"></td>
                    <td>
                        <span class=""><?=$pd['dealNo']?></span>
                    </td>
                    <td class="tg-kr94 current_stock_td"><?= ($pd['isis_stks'] + $pd['stocks_intransit']) - $pd['stocks_intransit']; ?>
                        <input type="hidden" value="<?= $pd['isis_stks']; ?>" name="bundle[cs_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
                    </td>
                    <td class="tg-kr94 stock_in_transit_td">
                        <?= ( $pd['stocks_intransit'] == '' ) ? 0 : $pd['stocks_intransit'] ?>
                        <input type="hidden" value="<?= ( $pd['stocks_intransit'] == '' ) ? 0 : $pd['stocks_intransit'] ?>" name="bundle[si_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
                    </td>
                    ...
                    <td class="tg-kr94 suggested_order_qty_td"><?= $pd['isis_order_stocks']; ?>
                        <input type="hidden" value="<?= $pd['isis_order_stocks']; ?>" name="bundle[soq_<?=$bundle_info['id']?>_<?= $pd['stock_id'] ?>]">
                    </td>
