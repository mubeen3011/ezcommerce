...
?>
<div class="tab-pane <?=(isset($_GET['poId'])) ? 'active' : '' ?> <?=(isset($_GET['selected_warehouse']) && $_GET['selected_warehouse'] == 7) ? 'active' : ''?>" id="f909-4">
    <form method="post" id="po_f909_4">
        <input type="hidden" name="category" value="<?php if (isset($_GET['category'])){echo $_GET['category'];}?>">
        <input type="hidden" name="warehouse" value="f909-4">
        <?= Yii::$app->controller->renderPartial('_poinfo', ['po' => $po,'warehouse'=>7]); ?>
                    ...
                    <table class="table table-bordered table-hover table-striped sticky-header">
                        ...
                        <tr>
                            <th class="form-checkbox-radio ">
                                <input type="checkbox" name="all-f909-4" checked value="all-f909-4" class="po_f909_4_chk">
                            </th>
                        ...
                        <tbody class="gridData f909-4-data">
                        <tr class="temp-tr-f909-4"></tr>
                        <?php
                        $SkusList=[];
                        $random_color=0;
                                    ...
                                    <tr class="" <?=$style?>>
                                        <td class="tg-kr94 form-checkbox-radio ">
                                            <?php if ($status == "" || (($po) && $po->po_status != 'Pending' && $po->po_status != 'Shipped') ): ?>
                                                <input type="checkbox" class="chk4" checked name="f909_4_po_skus[]"
                                                       value="<?= $pd['stock_id'] ?>">
                                            <?php endif;
                                        ...
                                        <td class="tg-kr94 current_stock_td"><?= ($pd['909_stks'] + $pd['fbl909_stocks_intransit']) - $pd['fbl909_stocks_intransit']; ?>
                                            <input type="hidden"
                                                   value="<?= $pd['909_stks']; ?>"
                                                   name="cs_<?= $pd['stock_id'] ?>"></td>
                                        <td class="tg-kr94 stock_in_transit_td">
                                            <?= ($pd['fbl909_stocks_intransit']=='') ? 0 : $pd['fbl909_stocks_intransit'] ?>
                                            &nbsp;<?= ($status == 'Shipped') ? '*' : '' ?>
                                            <input type="hidden"
                                                   value="<?= ($pd['fbl909_stocks_intransit']=='') ? 0 : $pd['fbl909_stocks_intransit'] ?>"
                                                   name="si_<?= $pd['stock_id'] ?>">
                                        </td>
                                        <td class="tg-kr94 master_carton_td">
                                            <?=isset($pd['master_cotton']) ? $pd['master_cotton'] : 0;?>
                                        </td>
                                        <td class="tg-kr94 suggested_order_qty_td"><?= $pd['fbl909_order_stocks']; ?>
                                            <input type="hidden"
                                                   value="<?= $pd['fbl909_order_stocks']; ?>"
...
//                                    $nc12 = ($po) ? $pod['nc12'][$sk] : $nc12;
                                            ...
                                            <tr <?=$style?>>
                                                <td class="tg-kr94 form-checkbox-radio ">
                                                    <?php if ($status == "" || (($po) && $po->po_status != 'Pending' && $po->po_status != 'Shipped') ): ?>
                                                        <input type="checkbox" class="chk4" checked name="f909_4_po_skus[]" value="<?= $pd['sid'].'_'.$skuId ?>">
                                                    <?php endif; ?>
                                                </td>
                                                <td class="tg-kr94">
                                                    &#x2192;&nbsp;<?= $ps->sku ?>
                                                ...
                                                <td class="tg-kr94"><?= $pd['fbl909_order_stocks']; ?>
                                                    <input type="hidden"
                                                           value="<?= $pd['fbl909_order_stocks']; ?>"
                                                ...
                                                <td class="tg-kr94"  style="padding: 5px"><input type="text" class="numberinput form-control custom-css-txt <?=$bind_foc_item?>"
                                                                                                 value="<?= $orderQty ?>"
                                                                                                 name="stock_<?=$pd['sid']?>_<?= $skuId ?>" readonly></td>
                                                <?php if ($po): ?>
                                                    <td class="tg-kr94" style="padding: 5px;">
                                ...
                                */
                                ...
                                foreach ( $values['Sku_list'] as $key=>$value ){
                                    ...
                                    <tr <?=$style?>  <?=($key==0) ? 'class="Bundle-Div"' : '' ?> <?=($key==0) ? 'id="Bundle-'.$values['Bundle_info'][0]['id'].'"' : ''?>>
                                        ...
                                        <td class="status_td">
                                            <?=$value['extra_information']['c'][0]['f909_stock_status']?>
                                        </td>
                                        <td class="variations_td">
                                        ...
                                        <td class="tg-kr94 threshold_td">
                                            <?=$value['extra_information']['c'][0]['threshold_org']?>
                                            <input type="hidden" value="<?=$value['extra_information']['c'][0]['threshold_org']?>" name="bundle[th_<?=$value['bundle']?>_<?=$value['extra_information']['c'][0]['stock_id']?>]">
                                        </td>
                                        <td>
                                            <br>
                                            <span>
                                        ...
                                        <td class="tg-kr94 current_stock_td">
                                            <?=($value['extra_information']['c'][0]['909_stks'] + $value['extra_information']['c'][0]['fbl909_stocks_intransit']) - $value['extra_information']['c'][0]['fbl909_stocks_intransit'] ?>
                                            <input type="hidden" value="<?=$value['extra_information']['c'][0]['909_stks']?>" name="bundle[cs_<?=$value['bundle']?>_<?=$value['extra_information']['c'][0]['stock_id']?>]">
                                        </td>
                                        <td class="tg-kr94 stock_in_transit_td">
        ...
        <div style="margin: 30px">
            <?php if ($po): ?>
                ...
                <?php if ($po->po_status == 'Pending'): ?>
                    ...
                    <!-- <button type="submit" class="btn btn-danger btn-f909-4-po pull-right" style="margin-right: 10px"
                             value="Force_ER">Force ER Push
                     </button>-->
                <?php elseif ($po->po_status != 'Shipped'): ?>

                    <input type="submit" class="btn btn-warning btn-f909-4-po pull-right" name="button_clicked" value="Finalize" />
                <?php endif; ?>

                <?php if ($po->po_status == 'Draft'): ?>
                    <input type="submit" class="btn btn-info btn-f909-4-po pull-right" style="margin-right: 10px;" name="button_clicked" value="Save" />
                <?php endif; ?>
                ...
                <a href="/stocks/po-print?poid=<?= $po->id ?>&warehouse=<?= $po->po_warehouse ?>"
                   style="margin-right: 10px" class="btn-f909-4-po pull-right isis_print">
                    <i style="font-size: 25px;" class="fa fa-print"></i> Print
                </a>
            <?php else: ?>
                <button type="submit" class="btn btn-info btn-f909-4-po pull-right initiate-po" value="Initiate PO">Initiate PO
                </button>
            <?php endif; ?>
        </div>
    </form>
    <?php if (!$po || $po->po_status == 'Draft'): ?>
        <div id="backToTheTut" data-warehouse="f909-4" class="add-prd badge label btn bg-blue-alt font-size-11 mrg10R float-right notification">
        ...
        <div id="backToTheTutBundle" data-warehouse="f909-4" class="add-bundle badge label btn bg-blue-alt font-size-11 mrg10R float-right notification">
            Add
            <br />Bundle
            <?php
            if (time() < 1546257992){
    ...
    <input type="hidden" id="skus_Already_in_pof909-4" value="<?=implode(',',$SkusList)?>" />
    <input type="hidden" id="bundle_already_in_pof909-4" value="<?=$bundle_ids?>" />
</div>
