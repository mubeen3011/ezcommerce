...
if(isset($couriers) && !empty($couriers)) { ?>
    <ul class="list-unstyled">
        <?php  foreach ($couriers as $courier) : ?>
            <?php if (isset($courier['name']) && $courier['type']!=null) : ?>
                <li class="media">
                    <img class="d-flex mr-3" src="<?= $courier['icon'];?>" width="40" >
                    <div class="media-body">
                        <h5 class="mt-0 mb-1"><?= $courier['name'];?></h5>
                        <span><?= $courier['description'];?></span>
                        <button class="btn-rounded pull-right btn btn-sm btn-success select-courier" data-courier-id="<?= $courier['id'];?>" data-courier-type="<?= $courier['type'];?>">
                                Select
                        </button>
                    </div>
                </li>
