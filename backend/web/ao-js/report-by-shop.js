...
$(document).ready( function () {
    $('.sales-performance-by-sku').DataTable({
        dom: 'Bfrtip',
        buttons: ['csv']
    });
});
...
$(function () {
    //// all year monthly /quarterly graph
    Morris.Bar({
    ...
    /******************************* category sales graph get on document ready ******************/
    $.ajax({
        type: "POST",
    ...
    /*******************************category sales graph filter when applied******************/
    ...
    /*******************************category sales chart population******************/
...
//for top 10 cntributors on click toggle variations
...
$('.save_image_bg').click(function() {
    html2canvas(document.getElementById('sales-by-shop-per-month-graph')).then(canvas => {
        var w = document.getElementById("sales-by-shop-per-month-graph").offsetWidth;
        var h = document.getElementById("sales-by-shop-per-month-graph").offsetHeight;
        save_image_as(canvas.toDataURL("image/jpeg"),new Date().toISOString().slice(0, 10) + 'sp_bar.jpg');
    }).catch(function(e) {
